...
def make_tokenizer(tokenizer_type, corpus, model_path=None, vocab_size=None, model_type='bpe', pad_token=0, character_coverage=1.0, command_tokens=None, type_tokens=None, **kwargs):
    ...
    elif tokenizer_class is BertSentencePieceTokenizer:
        return BertSentencePieceTokenizer(model_path, **kwargs)
...
class BertSentencePieceTokenizer(Tokenizer):
    """
    Loads a pretrained WordPiece tokenizer from `cache_dir` for tokenization
    in BERT training. Default to bert-large-uncased tokenizer.
    """
    def __init__(self, model_path=None, tokenizer_model_type="bpe", cache_dir=None, **kwargs):
        # default to bert-large-uncased tokenizer
        self.model_path = model_path
        if torch.distributed.get_rank() == 0:
            print('loading BertSentencePieceTokenizer (', tokenizer_model_type, ') from cache_dir ', cache_dir)
        #do_lower_case = not ('-cased' in tokenizer_model_type or 'chinese' in tokenizer_model_type)
        self._tokens = []
        self._vocab = {}
        self.load_spm_model()
        ...
        self.num_type_tokens = 2
        self._command_tokens = [
            CommandToken('pad', '[PAD]', self._vocab['[PAD]']),
            CommandToken('ENC', '[CLS]', self._vocab['[CLS]']),
            CommandToken('MASK', '[MASK]', self._vocab['[MASK]']),
            CommandToken('unk', '[UNK]', self._vocab['[UNK]']),
            CommandToken('sep', '[SEP]', self._vocab['[SEP]']),
            CommandToken('title', '[Title]', self._vocab['[Title]']),
            CommandToken('snippet', '[Snippet]', self._vocab['[Snippet]']),
            CommandToken('url', '[Url]', self._vocab['[Url]']),
        ]
        ...
        self._tokens = list(self._vocab.keys())
    ...
    def load_spm_model(self):
        ...
        if os.path.exists(self.model_path):
            self.spm_model_path = os.path.join(self.model_path, "sentencepiece.bpe.model")
            self.vocab_path = os.path.join(self.model_path, "vocab.txt")
            self.market_path = os.path.join(self.model_path, "market.txt")
        else:
            print("The tokenizer path ", self.model_path, " does not exist!!!")
        self.sp_tokenizer = spm.SentencePieceProcessor()
        self.sp_tokenizer.Load(self.spm_model_path)
        self._vocab = self.load_vocab(self.vocab_path)
        self._language_region =  self.load_vocab(self.market_path).keys()
        self.vocab_words = list(self._vocab.keys())
        self.ids_to_tokens = collections.OrderedDict(
            [(ids, tok) for tok, ids in self._vocab.items()])
        self.vocab_ids = self.convert_tokens_to_ids(self.vocab_words)

        self.vocab_size = len(self._vocab)

    def get_language_token_id(self, language):
        if 'lang_' + language.lower() in self._language_region:
            return self._vocab['lang_' + language.lower()]
        else:
            return self._vocab['lang_unk']
    
    def get_region_token_id(self, region):
        if 'region_' + region.lower() in self._language_region:
            return self._vocab['region_' + region.lower()]
        else:
            return self._vocab['region_unk']
    ...
    def tokenize(self, text):
        split_tokens = ["[UNK]" if x not in self._vocab else x for x in self.sp_tokenizer.EncodeAsPieces(text)]
        return split_tokens
    ...
    def DecodeIds(self, Ids, type_token=False):
        ...
        Tokens = []
        for Id in Ids:
            Tokens.append(self.ids_to_tokens[Id] if Id != -1 else '-1')
