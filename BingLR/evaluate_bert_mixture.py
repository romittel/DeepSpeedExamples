...
def get_batch(data_iterator, args, timers):
    ...
    sample_id = data_b['sample_id'].view(batch_size, num_urls).long()
    # Get the masks and postition ids.
...
def forward_step(data_iterator, model, args, timers):
    ...
    loss = loss + moe_loss + hrs_loss + click_loss
    
    return loss,  hrs_scores, hrslabels, sample_id
...
def train_step(data_iterator, model, optimizer, lr_scheduler,
    ...
    lm_loss, _, _ = forward_step(data_iterator, model, args, timers)
    timers('forward').stop()
    ...
    while iteration < args.train_iters:
        ...
        if args.eval_interval and iteration % args.eval_interval == 0 and args.do_valid:
            ...
            if val_xdcg_at_3 is None:
                ...
                save_checkpoint(100, model, optimizer, lr_scheduler, args)
...
def evaluate(data_iterator, model, args, timers, verbose=False):
    ...
    with torch.no_grad():
        for _ in range(1000):
            ...
            _, hrs_scores, _, sample_id = forward_step(data_iterator, model, args, timers)
            if cnt % 10 == 0:
                print("cnt= ", cnt, " hrs_scores= ", hrs_scores[0])
            cnt += 1
            hrs_scores_value = hrs_scores.view(-1, args.num_urls).detach().cpu().numpy()
            sample_id_value = sample_id.view(-1).cpu().numpy()
            for i, id in enumerate(sample_id_value):
                if id not in all_scores:
                    all_scores[id] = hrs_scores_value[i]
                else:
...
def evaluate_and_print_results(prefix, data_iterator, model,
    ...
    all_scores = evaluate(data_iterator, model, args, timers, verbose)
    all_data = {}
    cnt = 0
    #if torch.distributed.get_rank() == 0:
    #    f = open(args.val_data_path, 'r', encoding='utf-8')
    #    for line in f:
    #        id = line["sample_id"]
    #        if id not in all_scores:
    #            print("Didn't find the sample id")
    #            return
    #        language = line["language"].lower()
    #        region = line["region"].lower()
    #        market = language + '-' + region
    #        query = line["query"]
    #        hrslabel = float(line['hrslabel'])
    #        if market not in all_data:
    #            all_data[market] = {}
    #        if query not in all_data[market]:
    #            all_data[market][query] = []
    #        all_data[market][query].append((hrslabel, all_scores[id]))
    #        if cnt % 10 == 0:
    #            print("cnt= ", cnt)
    #        cnt += 1
    #    xdcg_at_3_sum = 0
    #    count = 0
    #    for market in all_data.keys():
    #        for query in all_data[market].keys():
    #            label_score_list = all_data[market][query]
    #            label_score_list = sorted(label_score_list, key=lambda x: x[1])
    #            xdcg = compute_xdcg(label_score_list, depth=3)
    #            xdcg_at_3_sum += xdcg[2]
    #            count += 1
    #    xdcg_at_3 = xdcg_at_3_sum / float(count)
    #    print("xdcg_at_3= ", xdcg_at_3)
    return None #xdcg_at_3
...
def main():
    ...
    if val_data is not None:
        test_data_iterator = iter(val_data)
    else:
