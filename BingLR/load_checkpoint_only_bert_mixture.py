...
def main():
    ...
    model.module.module.module.word_embeddings.weight = torch.nn.Parameter(torch.cat((torch.tensor(d_binglr['bert.embeddings.word_embeddings.weight'][j * emb_per_gpu : (j + 1) * emb_per_gpu,:], device=wts.device, dtype=wts.dtype), torch.zeros((num_embeddings_per_partition - emb_per_gpu, embedding_dim), device=wts.device, dtype=wts.dtype))), requires_grad=True)
            
    # position_embeddings
    wts = model.module.module.module.position_embeddings.weight
    model.module.module.module.position_embeddings.weight = torch.nn.Parameter(torch.tensor(d_binglr['bert.embeddings.position_embeddings.weight'], device=wts.device, dtype=wts.dtype), requires_grad=True)
            
    # input_layernorm
    wts = model.module.module.module.input_layernorm.weight
    model.module.module.module.input_layernorm.weight = torch.nn.Parameter(torch.tensor(d_binglr['bert.embeddings.LayerNorm.weight'], device=wts.device, dtype=wts.dtype), requires_grad=True)
    wts = model.module.module.module.input_layernorm.bias
    model.module.module.module.input_layernorm.bias = torch.nn.Parameter(torch.tensor(d_binglr['bert.embeddings.LayerNorm.bias'], device=wts.device, dtype=wts.dtype), requires_grad=True)
    ...
    for i in range(num_of_layers):
        ...
        model.module.module.module.transformer.layers[i].self_output.dense.weight = torch.nn.Parameter(torch.tensor(d_binglr['bert.encoder.layer.' + str(i) + '.attention.output.dense.weight'][:,j * hp : (j+1) * hp], device=wts.device, dtype=wts.dtype), requires_grad=True)
        wts = model.module.module.module.transformer.layers[i].self_output.dense.bias
        model.module.module.module.transformer.layers[i].self_output.dense.bias = torch.nn.Parameter(torch.tensor(d_binglr['bert.encoder.layer.' + str(i) + '.attention.output.dense.bias'], device=wts.device, dtype=wts.dtype), requires_grad=True)
                
        #self_output.layernorm
        wts = model.module.module.module.transformer.layers[i].self_output.layernorm.weight
        model.module.module.module.transformer.layers[i].self_output.layernorm.weight = torch.nn.Parameter(torch.tensor(d_binglr['bert.encoder.layer.' + str(i) + '.attention.output.LayerNorm.weight'], device=wts.device, dtype=wts.dtype), requires_grad=True)
        wts = model.module.module.module.transformer.layers[i].self_output.layernorm.bias
        model.module.module.module.transformer.layers[i].self_output.layernorm.bias = torch.nn.Parameter(torch.tensor(d_binglr['bert.encoder.layer.' + str(i) + '.attention.output.LayerNorm.bias'], device=wts.device, dtype=wts.dtype), requires_grad=True)
