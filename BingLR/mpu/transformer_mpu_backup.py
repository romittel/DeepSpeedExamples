...
class BertParallelTransformerLayer(torch.nn.Module):
    ...
    def __init__(self,
        ...
        if num_experts == 1:
            ...
            # Use the DeepSpeed API to use MoE layer and experts.
            # -- sharding, comm. and parameter handling will be done inside DeepSpeed 
            self.mlp = MoE(hidden_size, output_dropout_prob, BertParallelMLP(hidden_size, output_dropout_prob, init_method, output_layer_init_method=output_layer_init_method), num_experts=num_experts, mpu=mpu)
    ...
    def forward(self, hidden_states, attention_mask):
        ...
        #    print("print torch mlp norm= ", torch.norm(mlp_output))
        #    print("print attention self output= ", torch.norm(attention_self_output))
        mlp_output = self.layernorm(mlp_output + attention_self_output)

        return mlp_output, moe_loss

class BertParallelMLP(torch.nn.Module):
    """MLP for GPT2.
    ...
    def __init__(self, hidden_size, output_dropout_prob, init_method,
                 output_layer_init_method=None, layernorm_epsilon=1.0e-12):
        super(BertParallelMLP, self).__init__()
        # Set output layer initialization if not provided.
        if output_layer_init_method is None:
            output_layer_init_method = init_method
        # Project to 4h.
...
class BertParallelTransformer(torch.nn.Module):
    """GPT-2 transformer.
    ...
    def __init__(self,
                 ...
                 use_scaled_init_for_output_weights=True,
                 num_experts=1):
        super(BertParallelTransformer, self).__init__()
        # Store activation checkpoiting flag.
        self.checkpoint_activations = checkpoint_activations
        self.checkpoint_num_layers = checkpoint_num_layers
        ...
        if use_scaled_init_for_output_weights:
            output_layer_init_method = scaled_init_method(init_method_std,
                                                          num_layers)
        def get_layer(i):
            return BertParallelTransformerLayer(
                hidden_size,
                num_attention_heads,
                attention_dropout_prob,
                output_dropout_prob,
